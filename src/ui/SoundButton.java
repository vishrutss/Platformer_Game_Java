...
public class SoundButton extends PauseButton {
    ...
    private void loadSoundImages() {
        BufferedImage temp = LoadSave.GetAsset(LoadSave.SOUND_BUTTONS);
        soundImages = new BufferedImage[2][3];
        for (int i = 0; i < soundImages.length; i++) {
            for (int j = 0; j < soundImages[i].length; j++) {
                soundImages[i][j] = temp.getSubimage(j * SOUND_BTN_SIZE_DEFAULT, i * SOUND_BTN_SIZE_DEFAULT,
                        SOUND_BTN_SIZE_DEFAULT, SOUND_BTN_SIZE_DEFAULT);
            }
        }
    }
